...
from app.core.exceptions import PlayerNotFoundException, TeamNotFoundException, DuplicateResourceException
...
def create_player(db: Session, player: PlayerCreate) -> Player:
    ...
    if existing_player:
        raise DuplicateResourceException(f"Player '{player.name}' already exists in this team")
