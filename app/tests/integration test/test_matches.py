...
class TestMatchEndpoints:
    ...
    def test_create_match_valid_data(self, client, test_db, authenticated_headers):
        ...
        response = client.post("/matches/", json=match_data, headers=authenticated_headers)
    ...
    def test_create_match_same_teams(self, client, test_db, authenticated_headers):
        ...
        # Create a team
        team = Team(name="Test Team", coach_name="Coach", founded_year=2000, home_ground="Stadium")
        test_db.add(team)
        test_db.commit()
    ...
    def test_update_match_valid_data(self, client, test_db, authenticated_headers):
        ...
        response = client.put(f"/matches/{match.id}", json=update_data, headers=authenticated_headers)
        assert response.status_code == 200
    ...
    def test_update_match_nonexistent(self, client, authenticated_headers):
        ...
        response = client.put("/matches/999999", json=update_data, headers=authenticated_headers)
        assert response.status_code == 404
        assert "Match not found" in response.json()["detail"]
    ...
    def test_get_matches_by_team(self, client, test_db, authenticated_headers):
        ...
        team_a = Team(name="Team A", coach_name="Coach A", founded_year=2000, home_ground="Stadium A")
        team_b = Team(name="Team B", coach_name="Coach B", founded_year=2001, home_ground="Stadium B")
        team_c = Team(name="Team C", coach_name="Coach C", founded_year=2002, home_ground="Stadium C")
        ...
        matches_with_team_a = [
            Match(team_a_id=team_a.id, team_b_id=team_b.id, match_date=datetime.now() + timedelta(days=1), venue="Stadium A", score_team_a=2, score_team_b=1),
            Match(team_a_id=team_c.id, team_b_id=team_a.id, match_date=datetime.now() + timedelta(days=2), venue="Stadium C", score_team_a=0, score_team_b=3),
        ]
        
        # Create match not involving team_a
        match_without_team_a = Match(
            team_a_id=team_b.id, team_b_id=team_c.id,
            match_date=datetime.now() + timedelta(days=3),
            venue="Stadium B", score_team_a=1, score_team_b=1
        )
        ...
        response = client.get(f"/matches/?team_id={team_a.id}", headers=authenticated_headers)
        if response.status_code == 200:
            response_data = response.json()
            # Should only return matches involving team_a
            for match in response_data:
                assert match["team_a_id"] == team_a.id or match["team_b_id"] == team_a.id
    ...
    def test_match_score_validation(self, client, test_db, authenticated_headers):
        ...
        assert response.status_code in [201, 400, 422]  # Depending on validation implementation
    ...
    def test_get_upcoming_matches(self, client, test_db, authenticated_headers):
        ...
        past_match = Match(
            ...
            match_date=datetime.now() - timedelta(days=7),
            venue="Past Stadium", score_team_a=2, score_team_b=1
        )
        future_match = Match(
            team_a_id=team_a.id, team_b_id=team_b.id,
            match_date=datetime.now() + timedelta(days=7),
            venue="Future Stadium", score_team_a=0, score_team_b=0
        )
