...
class TestPlayerEndpoints:
    ...
    def test_get_player_by_id_existing(self, client, test_db, authenticated_headers):
        ...
        team = Team(
            name="Test FC", coach_name="Coach", founded_year=2000, home_ground="Stadium"
        )
        test_db.add(team)
        test_db.commit()
        test_db.refresh(team)

        player = Player(team_id=team.id, name="Test Player", position="Forward", age=25)
        test_db.add(player)
        test_db.commit()
    ...
    def test_create_player_valid_data(self, client, test_db, authenticated_headers):
        ...
        response = client.post(
            "/players/", json=player_data, headers=authenticated_headers
        )
    ...
    def test_update_player_valid_data(self, client, test_db, authenticated_headers):
        ...
        player = Player(
            team_id=team.id, name="Original Player", position="Forward", age=25
        )
        test_db.add(player)
        test_db.commit()
        test_db.refresh(player)
        ...
        response = client.put(
            f"/players/{player.id}", json=update_data, headers=authenticated_headers
        )
        assert response.status_code == 200
    ...
    def test_update_player_nonexistent(self, client, authenticated_headers):
        ...
        response = client.put(
            "/players/999999", json=update_data, headers=authenticated_headers
        )
        assert response.status_code == 404
        assert "Player not found" in response.json()["detail"]
    ...
    def test_delete_player_existing(self, client, test_db, authenticated_headers):
        ...
        player = Player(
            team_id=team.id, name="Delete Player", position="Forward", age=25
        )
        test_db.add(player)
        test_db.commit()
        test_db.refresh(player)
    ...
    def test_get_players_by_team(self, client, test_db, authenticated_headers):
        ...
        response = client.get(
            f"/players/?team_id={team1.id}", headers=authenticated_headers
        )
    ...
    def test_player_position_validation(self, client, test_db, authenticated_headers):
        ...
        response = client.post(
            "/players/", json=invalid_player_data, headers=authenticated_headers
        )
